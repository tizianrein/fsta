...
export default async function handler(req, res) {
  ...
  try {
    const { prompt, modelJson, files, model } = req.body;
    ...
    const selectedModel = model || 'gemini-2.5-pro'; // Default to 1.5 Pro if not provided
    ...
    const googleApiUrl = `https://generativelanguage.googleapis.com/v1beta/models/${selectedModel}:generateContent?key=${apiKey}`;
